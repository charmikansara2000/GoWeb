...
func Search(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	trainNo := r.URL.Query()["TrainNo"]
	trainName := r.URL.Query()["TrainName"]
	aTime := r.URL.Query()["ATime"]
	dTime := r.URL.Query()["DTime"]
	stName := r.URL.Query()["StName"]
	...
	if trainNo != nil {
		...
		cursor, err = collection.Find(context.Background(), bson.D{{"TrainNo", trainNo[0]}})
		if err != nil {
			log.Fatal(err)
		}
		fmt.Println(cursor)
	...
	if trainName != nil {
		cursor, err = collection.Find(context.Background(), bson.D{{"TrainName", trainName[0]}})
		if err != nil {
			log.Fatal(err)
		}
	}
	if aTime != nil {
		cursor, err = collection.Find(context.Background(), bson.D{{"ATime", aTime[0]}})
		if err != nil {
			log.Fatal(err)
		}
	}
	if dTime != nil {
		cursor, err = collection.Find(context.Background(), bson.D{{"DTime", dTime[0]}})
		if err != nil {
			log.Fatal(err)
		}
	}
	if stName != nil {
		cursor, err = collection.Find(context.Background(), bson.D{{"StName", stName[0]}})
		if err != nil {
			log.Fatal(err)
		}
	}
